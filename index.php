<!-- 改变所有html文件为php文件   cmd中找到当前目录输入 ren *.php *.php -->
<?php 
   // 引入数据库文件
   ...
   // // 注意** 命名数组不能和其他引用文件中的数据库数组名相同，否则会报undefined index错误
   // $array = [];
   ...
   $connect = connect();
   // sql语句
   $index_sql = 'select p.title, p.created, p.content, p.views, p.likes,p.feature, c.name, u.nickname,
            (select count(id) from comments where post_id = p.id) as commentsCount 
             from posts p         
            left join categories c on c.id = p.category_id
            left join users u on u.id = p.user_id
            order by p.created desc limit 5';
   // 执行sql语句,注意,命名数组不能和其他引用文件中的数据库数组名相同，否则会报undefined index错误
   $index_arr = query($connect, $index_sql); 
 ?>
      ...
      <div class="panel new">
        ...
        <?php 
           foreach ($index_arr as $key => $value) { ?>            
              ...
              <div class="main">
                 <p class="info"><?php echo $value['nickname'] ?> 发表于 <?php $value['created'] ?></p> 
